let
	...
	load = { src, replacements ? {} }:
		let
			...
			flake =
				let result = if !(builtins.pathExists lockFile) then
						# TODO: Decide if unlocked flakes should be supported.
						builtins.throw ''Missing lock file "${lockFile}" is not supported.''
